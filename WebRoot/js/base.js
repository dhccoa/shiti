		...
		if(args.indexOf(' ')!=-1){
			...
			for(var i=0;i<elements.length;i++){
			 ...
			 switch(elements[i].charAt(0)){
				...
				node=childElements;//保存父节点，因为childElements要清理，所以要创建node数组
				break;
...
Base.prototype.getTagName = function (tag,parentNode) {
	...
	var tags = node.getElementsByTagName(tag);//通过父节点来获取
...
Base.prototype.find=function(str){
	...
	this.elements=childElements;//将字节点替换父节点
	return this;
}
...
Base.prototype.resize = function (fn) {
	for(var i=0;i<this.elements.length;i++){
		...
		addEvent(window,'resize',function(){
			...
			}else if(element.offsetTop>getInner().height-element.offsetHeight){
...
Base.prototype.animate=function(obj){
	for(var i=0;i<this.elements.length;i++){
		var element=this.elements[i];
		var attr=obj['attr']=='x'?'left':obj['attr']=='y'?'top':obj['attr']=='w'?'width':obj['attr']=='h'?'height':obj['attr']=='o'?'opacity':obj['attr']!=undefined?obj['attr']:'left';//可选，left和top两种，默认是left
	...
	(target-parseInt(getStyle(element,attr)))/speed;
			...
			if(attr=='opacity'){
				...
				if(parseInt(target)!=parseInt(getStyle(element,attr)))flag=false;//动画没有 执行完毕，flag为false		
			}
